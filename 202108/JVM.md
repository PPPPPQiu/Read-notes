...
## 堆溢出  
### 内存溢出  
OutOfMemoryError异常,它俗理解就是内存不够，通常在运行大型程序时发生，当程序所需要的内存远远超出了JVM内存所承受大小，就会报出OutOfMemoryError异常(称为OOM异常)。   
内存溢出是指当我们新建一个实力对象时，实例对象所需占用的内存空间大于堆的可用空间。    
可以通过Jstack工具和图形化工具JConsole工具查询到发生异常的具体区域。  
### 内存泄漏  
